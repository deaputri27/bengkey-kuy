...
const BASE_URL = 'https://077a-139-228-111-126.ap.ngrok.io'


export default function AuthMitra() {
  ...
  const imageAnimatedStyle = useAnimatedStyle(() => {
    let interpolation
    if (isRegistering) {
      interpolation = interpolate(imagePosition.value, [0, 1], [(-height / 2) - 200, 0]);
    } else {
      interpolation = interpolate(imagePosition.value, [0, 1], [(-height / 2), 0]);
    }
    return {
  ...
  const useAuthStore = create((set) => ({
    ...
    loginUser: async (email, password) => {
      try {
        const response = await axios.post(`${BASE_URL}/partners/login`, { email, password });
        console.log(response.data.access_token, "<<<<< ini lhoo");
        await AsyncStorage.setItem('accessToken', response.data.access_token);
        navigation.navigate('DrawerSide')
      } catch (error) {
        console.log(error)
      }
    },
    registerUser: async (partnerName, email, password, phoneNumber,address,imageUrl) => {
      try {
        // console.log(partnerName, email, password, phoneNumber,address,imageUrl);
        const response = await axios.post(`${BASE_URL}/partners/register`, { partnerName, email, password, phoneNumber,address,imageUrl });
        await AsyncStorage.setItem('accessToken', response.data.accessToken);
        navigation.navigate('AuthMitra')
        console.log('feri biintaro12');
      } catch (error) {
  ...
  const handleLogin = async () => {
    ...
    await loginUser("Honda@mail.com", "123456");
  };
  ...
  const handleRegister = async () => {
    ...
    await registerUser(partnerName, email, password, phoneNumber,address,imageUrl)
  };
